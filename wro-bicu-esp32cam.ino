...
String mensaje;               // Variable con el dato que se envia al controlador
int mensaje_length;           // Tamaño del string de dato para el buffer de envío
...
void setup() {
  ...
  servidor.on("/modo/", [](){
    servidor.send(200,"application/json",String("{'modo':'"+modo+"'}"));
    });

  servidor.on(UriBraces("/codigo-qr/{}"), [](){
    String codigos = servidor.pathArg(0);
    getMensaje(codigos);
    });

  servidor.on(UriBraces("/color/{}"), [](){
    String color = servidor.pathArg(0);
    getMensaje(color);
    });
...
void procesar(){
  char val[mensaje_length];
  mensaje.toCharArray(val,mensaje_length);
  WireSlave.write(val);
}

void getMensaje(String dato){
  modo = "idle";                                            // Cambiamos el estado antes de enviar la respuesta
  servidor.send(200,"text/plain", "Datos recibidos");       // Enviamos respuesta para cambiar el estado del script
  digitalWrite(PIN_FLASH, LOW);                             // Apagamos el flash
  mensaje_length = dato.length();                           // Obtenemos el tamaño del dato para el buffer
  mensaje = dato;                                           // Pasamos el dato a la variable global
  Serial.println(dato);
}
